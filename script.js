...
Upload one or more clear and high-quality images that best illustrate the innovation, its components, or its application.`,
  ...
  const genreColorMap = {
    "Machine Learning": "#E0E7FF", // Light Indigo
    BioTechnology: "#DCFCE7", // Light Green
    "Artificial Intelligence": "#F3E8FF", // Light Purple
    "Data Science": "#FEF3C7", // Light Amber
    "Information Technology": "#DBEAFE", // Light Blue
    "Cyber Security": "#FEE2E2", // Light Red
    default: "#fff9ea", // Default Color
  };
  ...
  const techSpecsUl = document.querySelector(
    ".Theme .technological-specs .list-content ul"
  );
  ...
  const featuresImagesContainer = document.getElementById(
    "featuresImagesContainer"
  );
  const endOfDocumentImagesContainer = document.getElementById(
  ...
  const recordsTablePlaceholder = document.getElementById(
  ...
  function captureCurrentEdits() {
    if (
      ...
      currentRecordIndex >= csvData.length ||
      !csvData[currentRecordIndex]
    ) {
      return;
    }
    const record = csvData[currentRecordIndex];

    if (genreSelect) record.Genre = genreSelect.value;

    if (techTitleDisplay) record.Title = techTitleDisplay.textContent;
    if (trlNumDisplay) record.TRLLevel = trlNumDisplay.textContent;
    if (docketDisplay)
      record.Docket =
        docketDisplay.textContent === "N/A" && !record.hasOwnProperty("Docket")
          ? ""
          : docketDisplay.textContent;
    if (patentStatusDisplay)
      record.PatentStatus = patentStatusDisplay.textContent;
    if (descriptionDisplay)
      record.DetailedDescription = descriptionDisplay.innerHTML;

    if (innovatorsDisplay) {
      if (
        innovatorsDisplay.textContent !== "N/A" &&
        innovatorsDisplay.textContent.trim() !== ""
      ) {
        record.Innovators = innovatorsDisplay.textContent
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s)
          .join(";");
    ...
    const captureListContent = (ulElement) => {
      if (!ulElement) return "";
      const items = [];
      const liSpans = ulElement.querySelectorAll(
      ...
      if (
        ...
        ulElement.firstElementChild.classList.contains("placeholder-item")
      ) {
        return "";
      }
      liSpans.forEach((span) => {
        items.push(span.innerHTML.trim());
      });
      ...
      if (
        ...
        const placeholderDefaultText =
          ulElement.dataset.defaultText || "New item - edit here";
        if (
          ulElement.firstElementChild.textContent.trim() ===
    ...
    if (techSpecsUl)
      record.TechnicalSpecifications = captureListContent(techSpecsUl);
  ...
  function formatMultiItemCellForExcel(cellValue, currentHeader) {
    ...
    if (currentHeader === "RelatedLinks") {
      return items
        .map((item) => (item.match(/https?:\/\/[^\s]+/gi) || [item])[0])
        .filter((url) => url)
        .join(" ; ");
    }
    return items.join(" ; ");
  ...
  function manuallyGenerateCsvStringForExcel(
    aoa,
          ...
          .map((cellValue, colIndex) => {
            let cellString =
              cellValue === null || typeof cellValue === "undefined"
                ? ""
                : String(cellValue);
            const headerNameForCurrentColumn =
              allHeadersInOutputOrder[colIndex];
            let applyForcedQuoting =
              rowIndex > 0 &&
              dataHeadersToForceQuote.includes(headerNameForCurrentColumn);
  ...
  function processExcelToCsvString(excelFileData) {
    try {
      if (typeof XLSX === "undefined") {
        alert("XLSX library (SheetJS) is not loaded.");
      ...
      if (!allDataAoa || allDataAoa.length === 0) {
        alert("Error: The Excel sheet is empty or unreadable.");
        return null;
      }

      const colIndicesForDesiredHeaders = DESIRED_OUTPUT_CSV_HEADERS.map(
        (desiredHeader) => {
          const indexInMasterList =
            ALL_ORIGINAL_EXCEL_HEADERS_IN_ORDER.indexOf(desiredHeader);
      ...
      const processedAndFilteredRows = dataRows.map((originalRowArray) => {
        ...
        DESIRED_OUTPUT_CSV_HEADERS.forEach((desiredHeader, i) => {
          ...
          let cellValue = "";
          if (
            originalColIndexToFetch !== -1 &&
            originalColIndexToFetch < originalRowArray.length
          ) {
            cellValue = originalRowArray[originalColIndexToFetch];
          }
          if (
            EXCEL_COLUMNS_TO_REFORMAT_FOR_MULTI_ITEMS.includes(desiredHeader)
          ) {
      ...
      const outputAoa = [
        DESIRED_OUTPUT_CSV_HEADERS,
        ...processedAndFilteredRows,
      ];
      return manuallyGenerateCsvStringForExcel(
        outputAoa,
        EXCEL_DATA_IN_COLUMNS_TO_FORCE_QUOTE,
        DESIRED_OUTPUT_CSV_HEADERS
      );
    } catch (error) {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (values.length === headersToUse.length) {
          ...
          console.warn(
            `Skipping line ${i + 1} in CSV: column count mismatch. Expected ${
              headersToUse.length
  ...
  function populateList(ulElement, dataString) {
    if (!ulElement) return;
    ulElement.innerHTML = "";
    const defaultTextForItem =
      ulElement.dataset.defaultText || "New item - edit here";
    const items =
      dataString &&
      dataString.trim() !== "" &&
      dataString.toUpperCase() !== "N/A"
        ? dataString
            .split(";")
            .map((item) => item.trim())
            .filter((item) => item)
  ...
  function handleRemoveImage(imageUrl, isCsvImage) {
    ...
    if (isCsvImage) {
      ...
      if (uploadedImagesPerRow[currentRecordIndex])
        uploadedImagesPerRow[currentRecordIndex] = uploadedImagesPerRow[
          currentRecordIndex
  ...
  function displayRecordsInTable(records) {
    ...
    records.forEach((record, index) => {
      ...
      const innovators = record.Innovators
        ? record.Innovators.split(";")
            .map((s) => s.trim())
            .filter((s) => s)
            .join(", ")
        : "N/A";
      ...
      tr.addEventListener("click", () => {
        ...
        const currentlySelectedInTable =
          table.querySelector("tr.selected-record");
  ...
  function displayRecord(index) {
    ...
    if (endOfDocumentImagesContainer)
      endOfDocumentImagesContainer.innerHTML = "";
    ...
    if (
      ...
      if (documentHeader)
        documentHeader.style.backgroundColor = genreColorMap.default;
    ...
    if (genreSelect && record.Genre) {
      genreSelect.value = record.Genre;
    } else if (genreSelect) {
      genreSelect.value = "";
    }
    if (documentHeader) {
      const selectedColor =
        genreColorMap[record.Genre] || genreColorMap.default;
      documentHeader.style.backgroundColor = selectedColor;
    }
    ...
    if (innovatorsDisplay) {
      const innovatorsText = record.Innovators
        ? record.Innovators.split(";")
            .map((s) => s.trim())
            .filter((s) => s)
            .join(", ")
        : "N/A";
      innovatorsDisplay.textContent =
        innovatorsText === "" || innovatorsText === "N/A"
          ? "N/A"
          : innovatorsText;
    }
    ...
    if (descriptionDisplay)
      descriptionDisplay.innerHTML = record.DetailedDescription || "N/A";
    ...
    const filteredCsvImages = csvImageUrlsFromRecord
      ...
      .map((url) => ({ url, isCsv: true }));
    const uploadedImageObjects = (uploadedImagesPerRow[index] || []).map(
      (url) => ({ url, isCsv: false })
    );
    const combinedImageObjects = [
      ...filteredCsvImages,
      ...
      if (targetContainer) {
        ...
        combinedImageObjects.forEach((imgObj) =>
          targetContainer.appendChild(
            createImageElementWrapper(imgObj.url, imgObj.isCsv)
          )
        );
      }
    }
    ...
    if (qrCodeContainer && record.QRCodes) {
      const qrLinks = record.QRCodes.split(";")
        .map((s) => s.trim())
        .filter((s) => s);
      if (qrLinks.length > 0) {
        qrLinks.forEach((link) => {
          if (typeof QRCode === "function") {
            ...
            try {
              ...
              itemDiv.textContent = "QR Err";
            }
            const btn = document.createElement("button");
            btn.className = "remove-qr-btn";
            btn.innerHTML = "&times;";
            ...
            p.textContent = `QR: ${link.substring(0, 15)}... (lib missing)`;
    ...
    if (downloadPdfButton)
      downloadPdfButton.disabled = index < 0 || csvData.length === 0;
      ...
      if (file) {
        ...
        const processCsvTextData = (csvText) => {
          try {
            ...
            if (csvData.length > 0) {
              displayRecordsInTable(csvData);
              if (recordsTablePlaceholder)
                recordsTablePlaceholder.style.display = "none";
              ...
              if (recordsTablePlaceholder) {
                recordsTablePlaceholder.textContent =
        ...
        if (fileName.endsWith(".xlsx") || fileName.endsWith(".xls")) {
          ...
          reader.onerror = (e) => {
            ...
            console.error("File read error (Excel):", e);
            currentRecordIndex = -1;
            displayRecord(-1);
            if (recordsTableContainer) recordsTableContainer.innerHTML = "";
            if (recordsTablePlaceholder) {
              recordsTablePlaceholder.textContent = "Failed to read file.";
              recordsTablePlaceholder.style.display = "block";
            }
          };
          ...
          reader.onerror = (e) => {
            ...
            console.error("File read error (CSV):", e);
            currentRecordIndex = -1;
            displayRecord(-1);
            if (recordsTableContainer) recordsTableContainer.innerHTML = "";
            if (recordsTablePlaceholder) {
          ...
          if (recordsTableContainer) recordsTableContainer.innerHTML = "";
          if (recordsTablePlaceholder) {
            recordsTablePlaceholder.textContent = "Unsupported file type.";
            recordsTablePlaceholder.style.display = "block";
          }
        }
      } else {
        ...
        if (recordsTablePlaceholder) {
          recordsTablePlaceholder.textContent =
            "No data loaded. Upload a file to see records.";
          recordsTablePlaceholder.style.display = "block";
        }
      }
  ...
  if (genreSelect && documentHeader) {
    genreSelect.addEventListener("change", () => {
      const selectedGenre = genreSelect.value;
      const newColor = genreColorMap[selectedGenre] || genreColorMap.default;
      documentHeader.style.backgroundColor = newColor;
    });
  }
  ...
  if (imageUploadInput) {
    imageUploadInput.addEventListener("change", async (event) => {
      if (currentRecordIndex < 0 || currentRecordIndex >= csvData.length) {
        alert("Please load data and select a record first.");
        if (event.target) event.target.value = "";
        return;
      }
  ...
  if (mainContentElement) {
    mainContentElement.addEventListener("click", function (event) {
      ...
      if (target.classList.contains("add-list-item-btn")) {
        ...
        captureCurrentEdits();
        const ulElement = mainContentElement.querySelector(
          target.dataset.listTargetSelector
        );
        if (ulElement) {
          const defaultText =
            ulElement.dataset.defaultText || "New item - edit here";
          let text =
            ulElement.children.length === 0 ||
            (ulElement.children.length === 1 &&
              ulElement.firstElementChild?.classList.contains(
                "placeholder-item"
              ))
              ? ((ulElement.innerHTML = ""), defaultText)
              : "Edit this item...";
          const newItemLi = createEditableListItem(ulElement, text, false);
          if (newItemLi)
            newItemLi.querySelector('span[contenteditable="true"]')?.focus();
        }
      } else if (
        target.classList.contains("delete-list-item-btn") ||
        target.closest(".delete-list-item-btn")
      ) {
        event.preventDefault();
        if (currentRecordIndex < 0) {
          alert("Select a record first.");
  ...
  if (downloadPdfButton) {
    downloadPdfButton.addEventListener("click", () => {
      if (currentRecordIndex < 0 || !csvData[currentRecordIndex]) {
      ...
      if (!element) {
        alert("Printable element .a4-sheet not found.");
        return;
      }
      const controlsContainer = document.querySelector(".controls-container");

      const shiftAmountPx = 0;
      const originalTransform = element.style.transform;
      if (controlsContainer) controlsContainer.style.display = "none";
      if (boldTooltip) boldTooltip.style.display = "none";

      const interactiveButtons = element.querySelectorAll(
        ".remove-image-btn, .delete-list-item-btn, .remove-qr-btn, .add-list-item-btn"
      );
      const btnOriginalDisplay = Array.from(interactiveButtons).map((btn) => ({
        el: btn,
      ...
      const editables = Array.from(
        element.querySelectorAll('[contenteditable="true"]')
      );
      ...
      const opt = {
        margin: [0, 0, 0, 0],
        filename: `${(csvData[currentRecordIndex].Title || "Untitled").replace(
          /[^a-z0-9]/gi,
          "_"
        )}.pdf`,
        ...
        html2canvas: {
          ...
          scrollY: 0,
          onclone: (doc) => {
            doc.querySelectorAll('[data-was-editable="true"]').forEach((el) => {
        ...
        jsPDF: {
          unit: "mm",
          format: [210, (element.scrollHeight * 25.4) / 96.0],
      ...
      const revertUI = () => {
        ...
        btnOriginalDisplay.forEach(
          (item) => (item.el.style.display = item.display)
        );
        editables.forEach((el) => {
      ...
      if (selection && selection.toString().trim() !== "") {
        ...
        if (container) {
          ...
          while (
            temp &&
            temp !== document.body &&
            temp !== a4Sheet.parentNode
          ) {
            if (temp.isContentEditable && a4Sheet.contains(temp)) {
        ...
        if (isEditableInSheet) {
          ...
          let left =
            ...
            window.scrollX +
            rect.width / 2 -
            boldTooltip.offsetWidth / 2;
          left = Math.max(
            window.scrollX + 5,
            Math.min(
              left,
              window.scrollX + window.innerWidth - boldTooltip.offsetWidth - 5
            )
          );
  ...
  if (generateQrButton && qrLinkInput && qrCodeContainer) {
    generateQrButton.addEventListener("click", () => {
      ...
      captureCurrentEdits();
      const record = csvData[currentRecordIndex];
      let links = record.QRCodes
        ? record.QRCodes.split(";")
            .map((s) => s.trim())
            .filter((s) => s)
        : [];
      ...
      links.push(linkText);
      record.QRCodes = links.join(";");
      displayRecord(currentRecordIndex);
      qrLinkInput.value = "";
    });
  } else {
    console.warn("Some QR elements missing (button, input, or container).");
  }
